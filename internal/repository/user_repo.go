...
func (r *UserRepo) GetByID(ctx context.Context, id int64) (*models.User, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, username, email, password_hash, is_admin, COALESCE(avatar_url, '') AS avatar_url, created_at, updated_at FROM users WHERE id = ?`, id,
	).Scan(&u.ID, &u.Username, &u.Email, &u.PasswordHash, &u.IsAdmin, &u.AvatarURL, &u.CreatedAt, &u.UpdatedAt)
...
func (r *UserRepo) CreateOAuthUser(ctx context.Context, username, email, avatarURL string) (*models.User, error) {
	log := beginDBOperation(ctx, userRepositoryName, "create_oauth_user",
		zap.String("username", username),
		zap.String("email", email),
	)
	tx, err := r.db.BeginTx(ctx, nil)
...
func (r *UserRepo) GetByUsername(ctx context.Context, username string) (*models.User, error) {
	...
	u := &models.User{}
	err := r.db.QueryRowContext(ctx,
		`SELECT id, username, email, password_hash, is_admin, COALESCE(avatar_url, '') AS avatar_url, created_at, updated_at FROM users WHERE username = ?`, username,
	).Scan(&u.ID, &u.Username, &u.Email, &u.PasswordHash, &u.IsAdmin, &u.AvatarURL, &u.CreatedAt, &u.UpdatedAt)
	if err == sql.ErrNoRows {
		log.complete(nil, zap.Bool("found", false))
		return nil, nil
...
func (r *UserRepo) GetByEmail(ctx context.Context, email string) (*models.User, error) {
	...
	u := &models.User{}
	err := r.db.QueryRowContext(ctx,
		`SELECT id, username, email, password_hash, is_admin, COALESCE(avatar_url, '') AS avatar_url, created_at, updated_at FROM users WHERE email = ?`, email,
	).Scan(&u.ID, &u.Username, &u.Email, &u.PasswordHash, &u.IsAdmin, &u.AvatarURL, &u.CreatedAt, &u.UpdatedAt)
	if err == sql.ErrNoRows {
		log.complete(nil, zap.Bool("found", false))
		return nil, nil
...
func (r *UserRepo) ListAll(ctx context.Context, page, limit int, search string) ([]models.User, int64, error) {
	...
	offset := (page - 1) * limit
	query := `SELECT id, username, email, password_hash, is_admin, COALESCE(avatar_url, '') AS avatar_url, created_at, updated_at FROM users`
	if search != "" {
	...
	for rows.Next() {
		var u models.User
		if err := rows.Scan(&u.ID, &u.Username, &u.Email, &u.PasswordHash, &u.IsAdmin, &u.AvatarURL, &u.CreatedAt, &u.UpdatedAt); err != nil {
			log.complete(err, zap.Int64("count", total))
...
func (r *UserRepo) UpdateAvatar(ctx context.Context, id int64, avatarURL string) error {
	log := beginDBOperation(ctx, userRepositoryName, "update_user_avatar",
		zap.Int64("user_id", id),
	)
	result, err := r.db.ExecContext(ctx,
		`UPDATE users SET avatar_url = ?, updated_at = ? WHERE id = ?`,
		avatarURL, time.Now().UTC().Format(time.RFC3339), id,
	)
	if err != nil {
		log.complete(err)
